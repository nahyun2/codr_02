...
## 리팩터링. `quiz.py` 가독성 개선 (`refactor/readability`)

**동기**: 이슈 9→18→추가 기능을 거치며 `play()` 하나가 "개수 입력 → 무작위 선택 → 출제/채점 → 힌트 처리 → 결과 출력 → 기록/최고점수 갱신 → 저장"을 전부 떠맡게 돼서 한눈에 파악하기 어려워짐. 동작은 그대로 두고 구조만 정리.

**개념 — 메서드 추출(Extract Method)**
- 긴 메서드를 "무엇을 하는지 이름이 설명해주는" 작은 메서드들로 쪼개는 리팩터링 기법. 각 조각은 한 가지 일만 하므로 이름만 보고도 흐름을 따라갈 수 있고, 테스트/재사용도 쉬워짐.
- `play()`를 5개로 분리:
  - `_ask_play_count(total)`: 몇 문제 풀지 입력받기
  - `_pick_random_quizzes(count)`: 무작위로 count개 고르기
  - `_run_round(quizzes)`: 실제 출제/채점 루프, 점수 반환
  - `_ask_answer(quiz)`: 정답 입력 + 힌트(0) 처리
  - `_record_result(score, total_played)`: 풀이 기록 남기고 최고점수 갱신
  - `play()` 자신은 이제 이 조각들을 순서대로 호출하는 "목차" 역할만 함.
- 밑줄로 시작하는 이름(`_ask_answer` 등)은 파이썬 관례상 "이 클래스 내부에서만 쓰는 헬퍼"라는 뜻(강제되는 규칙은 아니고 관례). `play()`처럼 밑줄 없는 이름은 `main.py`에서 직접 호출하는 공개 메서드.

**개념 — 중복 제거**
- `load_state()`에서 "파일 없음"과 "파일 손상" 두 경우 모두 `default_quizzes()`로 되돌리고 `save_state()`를 부르는 코드가 똑같이 반복되고 있었음 → `_reset_to_default_data()`로 묶어서 한 곳만 고치면 되게 함.
- `add_quiz()`에서 "빈 값이면 다시 물어보기"가 문제/선택지마다 반복되던 것을 모듈 함수 `_ask_nonempty(prompt, empty_message)`로 추출. 문제와 선택지는 안내 문구만 다르므로 그 두 개를 파라미터로 받게 함 — 출력 문구는 리팩터링 전과 완전히 동일.
- 선택지 개수 `4`가 `range(1, 5)`와 `read_int(..., 1, 4)` 두 곳에 흩어져 있던 걸 `NUM_QUIZ_CHOICES = 4` 상수로 모음 — 나중에 선택지 개수를 바꾸고 싶으면 한 곳만 수정하면 됨.

**개념 — 메서드 그룹 정리**
- `QuizGame` 안의 메서드 순서를 "저장/불러오기 → 메뉴 → 퀴즈 풀기(+ 헬퍼들) → 퀴즈 관리(추가/목록/삭제) → 점수 확인" 순으로 재배치하고, `# ---------- 구간 ----------` 주석으로 구획을 나눔. 기능은 그대로, 위에서 아래로 읽었을 때 "상태 다루는 부분 → 사용자가 보는 기능들" 순서로 자연스럽게 읽히도록.

**검증**: 리팩터링 전/후로 똑같은 시나리오(퀴즈 풀기 - 힌트/부분풀이/범위밖입력, 퀴즈 추가, 목록, 삭제 취소/삭제, 점수 확인, 파일 없음/손상 시 폴백)를 실행해서 출력이 한 글자도 다르지 않은 것을 확인함. 즉 **동작은 100% 동일하고 구조만 바뀜**.

**추가 정리 — 중복 문자열 상수화**: `grep`으로 파일 전체에서 두 번 이상 등장하는 문자열 리터럴을 찾아보니, `"등록된 퀴즈가 없습니다."`가 `play()` / `list_quizzes()` / `delete_quiz()` 세 군데에 토씨 하나 안 틀리고 반복되고 있었음. 모듈 상수 `NO_QUIZZES_MESSAGE`로 뽑아서 세 곳 모두 `print(NO_QUIZZES_MESSAGE)`로 교체 — 나중에 문구를 바꾸고 싶으면 한 곳만 고치면 되고, 세 군데가 항상 같은 문구를 쓴다는 것도 코드만 보고 보장됨(리터럴 3개였다면 하나만 고치고 나머지를 놓치는 실수가 가능했음). 퀴즈를 전부 지운 뒤 풀기/목록/삭제 메뉴를 각각 호출해서 세 곳 모두 동일한 메시지가 출력되는 것을 확인함.
